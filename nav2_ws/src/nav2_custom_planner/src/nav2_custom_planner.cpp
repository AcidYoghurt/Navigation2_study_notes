...
namespace nav2_planner_system
{
    ...
    nav_msgs::msg::Path CustomPlanner::createPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal, std::function<bool()> cancel_checker)
    {
        ...
        int total_number_of_loop =                                                                                      // 计算出每多少米放一个点
            std::hypot(goal.pose.position.x - start.pose.position.x,goal.pose.position.y - start.pose.position.y)   // hypot：计算两点之间的直线距离，单位为m
            / interpolation_resolution_;                                                                                // interpolation_resolution_是路径插值分辨率，当分辨率为0.1，就表示每0.1米放一个点
        ...
        for (int i = 0; i < total_number_of_loop; ++i)
        {
            ...
            {
                RCLCPP_WARN(node_->get_logger(), "规划被取消（插值阶段）");
                throw nav2_core::PlannerCancelled("规划被取消");
            }
        ...
        for (geometry_msgs::msg::PoseStamped pose : global_path.poses)
        {
            ...
            if (costmap_2d_->worldToMap(pose.pose.position.x, pose.pose.position.y, mx, my))
            {
                unsigned char cost = costmap_2d_->getCost(mx, my); // 获取对应栅格的代价值
                // 如果存在致命障碍物则抛出异常（LETHAL_OBSTACLE表示致命障碍物）
